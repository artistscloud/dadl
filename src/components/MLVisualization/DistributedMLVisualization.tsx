...
const TabButton: React.FC<TabButtonProps> = ({ active, onClick, icon, label }) => {
  return (
    <button 
      className={`tab-button flex items-center px-4 py-3 font-medium rounded-t-lg transition-all duration-300 ${
        active 
          ? 'bg-white text-mlblue shadow-sm active' 
          : 'bg-mlgray-100 text-mlgray-600 hover:bg-mlgray-200'
      }`}
      onClick={onClick}
    >
...
const TabsNavigation: React.FC = () => {
  const { activeTab, setActiveTab } = useMLData();
          ...
          <div className="bg-gradient-to-r from-mlblue to-mlpurple rounded-2xl overflow-hidden shadow-lg animate-fade-in">
            <div className="p-8 text-white">
              <h2 className="text-2xl md:text-3xl font-bold mb-2 tracking-tight">
                Distributed Machine Learning with Density-Aware Optimization
              </h2>
